    ...
    this.domCreate = function() {
        ...
        // for ( var pg = 0; pg < 2; pg++ ) {
            for ( var act = 0; act < 9; act++ ) {
                ...
                thumb.id = 'sceneThumb_0_' + actCnt;
                thumb.className = 't' + String(Math.ceil(act + 1));
                $('.pgr_0').append(thumb);
                $('#sceneThumb_0_' + actCnt).attr('accesskey', actCnt);

                var $th = $('#sceneThumb_0_' + actCnt).clone();
                $th.attr('id', 'sceneThumb_1_' + actCnt);
        ...
        $('#carousel img').bind('click', function(event) {
            ...
            $('#toplvl_' + levelCurrent).attr('class', $('#toplvl_' + levelCurrent).attr('class') + ' lvlon');
